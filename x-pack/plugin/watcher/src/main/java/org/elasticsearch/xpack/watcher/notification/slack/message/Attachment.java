...
public class Attachment implements MessageElement {
    ...
    public static Attachment render(TextTemplateEngine engine, Map<String, Object> model, SlackMessageDefaults.AttachmentDefaults defaults,
                                    Attachment attachment) {
        String fallback = attachment.fallback != null ? engine.render(new TextTemplate(attachment.fallback), model) : defaults.fallback;
        String color = attachment.color != null ? engine.render(new TextTemplate(attachment.color), model) : defaults.color;
        String pretext = attachment.pretext != null ? engine.render(new TextTemplate(attachment.pretext), model) : defaults.pretext;
        String authorName = attachment.authorName != null ? engine.render(new TextTemplate(attachment.authorName), model)
            : defaults.authorName;
        String authorLink = attachment.authorLink != null ? engine.render(new TextTemplate(attachment.authorLink), model)
            : defaults.authorLink;
        String authorIcon = attachment.authorIcon != null ? engine.render(new TextTemplate(attachment.authorIcon), model)
            : defaults.authorIcon;
        String title = attachment.title != null ? engine.render(new TextTemplate(attachment.title), model) : defaults.title;
        String titleLink = attachment.titleLink != null ? engine.render(new TextTemplate(attachment.titleLink), model) : defaults.titleLink;
        String text = attachment.text != null ? engine.render(new TextTemplate(attachment.text), model) : defaults.text;
        String imageUrl = attachment.imageUrl != null ? engine.render(new TextTemplate(attachment.imageUrl), model) : defaults.imageUrl;
        String thumbUrl = attachment.thumbUrl != null ? engine.render(new TextTemplate(attachment.thumbUrl), model) : defaults.thumbUrl;
        Field[] fields = null;
        if (attachment.fields != null) {
            fields = new Field[attachment.fields.length];
            for (int i = 0; i < fields.length; i++) {
                fields[i] = Field.render(engine, model, defaults.field, attachment.fields[i]);
            }
        }
        String[] markdownFields = null;
        if (attachment.markdownSupportedFields != null) {
            markdownFields = new String[attachment.markdownSupportedFields.length];
            for (int i = 0; i < attachment.markdownSupportedFields.length; i++) {
                markdownFields[i] = engine.render(new TextTemplate(attachment.markdownSupportedFields[i]), model);
            }
        }
        List<Action> actions = new ArrayList<>();
        if (attachment.actions != null && attachment.actions.isEmpty() == false) {
            for (Action action : attachment.actions) {
                actions.add(Action.render(engine, model, action));
            }
        }
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                if (token == XContentParser.Token.START_ARRAY) {
                    ...
                    try {
                        fields = new Field[]{Field.parse(parser)};
                    } catch (ElasticsearchParseException pe) {
                        throw new ElasticsearchParseException("could not parse message attachment. failed to parse [{}] field", pe,
                            XField.FIELDS);
                    }
                ...
                if (token == XContentParser.Token.START_ARRAY) {
                    ...
                    markdownFields = list.toArray(new String[list.size()]);
                } else {
                    try {
                        markdownFields = new String[]{parser.text()};
                    } catch (ElasticsearchParseException pe) {
                        throw new ElasticsearchParseException("could not parse message attachment. failed to parse [{}] field", pe,
                            XField.MARKDOWN_IN);
                    }
    ...
    public static class Builder {
        ...
        private List<String> markdownFields = new ArrayList<>();
        ...
        public Attachment build() {
            Field[] fields = this.fields.isEmpty() ? null : this.fields.toArray(new Field[this.fields.size()]);
            String[] markdownFields =
                this.markdownFields.isEmpty() ? null : this.markdownFields.toArray(new String[this.markdownFields.size()]);
            return new Attachment(fallback, color, pretext, authorName, authorLink, authorIcon, title, titleLink, text, fields, imageUrl,
                thumbUrl, markdownFields, actions);
        }
    }
